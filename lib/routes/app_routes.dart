part of 'app_pages.dart';
// DO NOT EDIT. This is code generated via package:get_cli/get_cli.dart

abstract class Routes {
  Routes._();

  // authentication view

  static const PHONE_AUTH_SCREEN = _Paths.PHONE_AUTH_SCREEN;
  static const PHONE_AUTH_OTP_SCREEN = _Paths.PHONE_AUTH_OTP_SCREEN;

  // admin dashboard view

  static const ADMIN_DASHBOARD_SCREEN = _Paths.ADMIN_DASHBOARD_SCREEN;
  static const ADMIN_CREATE_COMPANY_PAGE = _Paths.ADMIN_CREATE_COMPANY_PAGE;

  //company dashboard view

  static const COMPANY_DASHBOARD_SCREEN = _Paths.COMPANY_DASHBOARD_SCREEN;
  static const COMPANY_CREATE_DASHBOARD = _Paths.COMPANY_CREATE_DASHBOARD;
  static const COMPANY_CREATE_FORM_PAGE = _Paths.COMPANY_CREATE_FORM_PAGE;

  // party bank view

  static const COMPANY_CREATE_LEDGER_PARTY_BANK_PAGE = _Paths.COMPANY_CREATE_LEDGER_PARTY_BANK_PAGE;
  static const VIEW_BANK_LEDGER_PROFILE_PAGE = _Paths.VIEW_BANK_LEDGER_PROFILE_PAGE;
  static const COMPANY_CREATE_LEDGER_DASHBOARD = _Paths.COMPANY_CREATE_LEDGER_DASHBOARD;

  // employee view

  static const CREATE_lEDGER_EMPLOYEE_AADHARKYC_PAGE = _Paths.CREATE_lEDGER_EMPLOYEE_AADHARKYC_PAGE;
  static const CREATE_lEDGER_EMPLOYEE_EKYC_PAGEVIEW = _Paths.CREATE_lEDGER_EMPLOYEE_EKYC_PAGEVIEW;
  static const CREATE_lEDGER_EMPLOYEE_MANUAL_KYC_PAGEVIEW = _Paths.CREATE_lEDGER_EMPLOYEE_MANUAL_KYC_PAGEVIEW;
  static const VIEW_lEDGER_EMPLOYEE_PROFILE = _Paths.VIEW_lEDGER_EMPLOYEE_PROFILE;

  // agent view

  static const CREATE_lEDGER_AGENT_AADHARKYC_PAGE = _Paths.CREATE_lEDGER_AGENT_AADHARKYC_PAGE;
  static const CREATE_lEDGER_AGENT_EKYC_PAGEVIEW = _Paths.CREATE_lEDGER_AGENT_EKYC_PAGEVIEW;
  static const CREATE_lEDGER_AGENT_MANUAL_KYC_PAGEVIEW = _Paths.CREATE_lEDGER_AGENT_MANUAL_KYC_PAGEVIEW;
  static const VIEW_lEDGER_AGENT_PROFILE = _Paths.VIEW_lEDGER_AGENT_PROFILE;

  // distributor view

  static const CREATE_lEDGER_DISTRIBUTOR_AADHARKYC_PAGE = _Paths.CREATE_lEDGER_DISTRIBUTOR_AADHARKYC_PAGE;
  static const CREATE_lEDGER_DISTRIBUTOR_EKYC_PAGEVIEW = _Paths.CREATE_lEDGER_DISTRIBUTOR_EKYC_PAGEVIEW;
  static const CREATE_lEDGER_DISTRIBUTOR_MANUAL_KYC_PAGEVIEW = _Paths.CREATE_lEDGER_DISTRIBUTOR_MANUAL_KYC_PAGEVIEW;
  static const VIEW_lEDGER_DISTRIBUTOR_PROFILE = _Paths.VIEW_lEDGER_DISTRIBUTOR_PROFILE;

  // create loan view

  static const COMPANY_CREATE_LOAN_DASHBOARD = _Paths.COMPANY_CREATE_LOAN_DASHBOARD;
  static const COMPANY_CREATE_LOAN_PAGE = _Paths.COMPANY_CREATE_LOAN_PAGE;
  static const COMPANY_VIEW_LOAN_PAGE = _Paths.COMPANY_VIEW_LOAN_PAGE;

  // create level view

  static const COMPANY_CREATE_LEVEL_PAGE = _Paths.COMPANY_CREATE_LEVEL_PAGE;

  // career view

  static const COMPANY_CAREER_PAGE_VIEW = _Paths.COMPANY_CAREER_PAGE_VIEW;
}

abstract class _Paths {

  // authentication view

  static const PHONE_AUTH_SCREEN = '/phoneAuth';
  static const PHONE_AUTH_OTP_SCREEN = '/phoneAuthOtp';

  // admin dashboard view

  static const ADMIN_DASHBOARD_SCREEN = '/adminDashboard';
  static const ADMIN_CREATE_COMPANY_PAGE = '/adminCreateCompanyPage';

  //company dashboard view

  static const COMPANY_DASHBOARD_SCREEN = '/companyDashboard';
  static const COMPANY_CREATE_DASHBOARD = '/companyCreateDashboard';
  static const COMPANY_CREATE_FORM_PAGE = '/companyCreateFormPage';

  // party bank view

  static const COMPANY_CREATE_LEDGER_PARTY_BANK_PAGE = '/companyCreateLedgerPartyBankPage';
  static const VIEW_BANK_LEDGER_PROFILE_PAGE = '/viewBankLedgerProfilePage';
  static const COMPANY_CREATE_LEDGER_DASHBOARD = '/companyCreateLedgerDashboardScreen';

  // employee view

  static const CREATE_lEDGER_EMPLOYEE_AADHARKYC_PAGE = '/companyCreateLedgerEmployeeAadharKycPage';
  static const CREATE_lEDGER_EMPLOYEE_EKYC_PAGEVIEW = '/companyCreateLedgerEmployeeEkycPageView';
  static const CREATE_lEDGER_EMPLOYEE_MANUAL_KYC_PAGEVIEW = '/companyCreateLedgerEmployeeManualKycPageView';
  static const VIEW_lEDGER_EMPLOYEE_PROFILE = '/viewLedgerEmployeeProfile';

  // agent view

  static const CREATE_lEDGER_AGENT_AADHARKYC_PAGE = '/companyCreateLedgerAgentAadharKycPage';
  static const CREATE_lEDGER_AGENT_EKYC_PAGEVIEW = '/companyCreateLedgerAgentEkycPageView';
  static const CREATE_lEDGER_AGENT_MANUAL_KYC_PAGEVIEW = '/companyCreateLedgerAgentManualKycPageView';
  static const VIEW_lEDGER_AGENT_PROFILE = '/viewLedgerAgentProfile';

  // distributor view

  static const CREATE_lEDGER_DISTRIBUTOR_AADHARKYC_PAGE = '/companyCreateLedgerDistributorAadharKycPage';
  static const CREATE_lEDGER_DISTRIBUTOR_EKYC_PAGEVIEW = '/companyCreateLedgerDistributortEkycPageView';
  static const CREATE_lEDGER_DISTRIBUTOR_MANUAL_KYC_PAGEVIEW = '/companyCreateLedgerDistributorManualKycPageView';
  static const VIEW_lEDGER_DISTRIBUTOR_PROFILE = '/viewLedgerDistributorProfile';

  // create loan view

  static const COMPANY_CREATE_LOAN_DASHBOARD = '/companyCreateLoanDashboardScreen';
  static const COMPANY_CREATE_LOAN_PAGE = '/companyCreateLoanPage';
  static const COMPANY_VIEW_LOAN_PAGE = '/companyViewLoanPage';

  // create level view

  static const COMPANY_CREATE_LEVEL_PAGE = '/companyCreateLevelPage';

  // career view

  static const COMPANY_CAREER_PAGE_VIEW = '/companyCareerPageView';

}
